...
# from extarget import NetSimpleConv
...
parser = argparse.ArgumentParser(description='PyTorch NetSimpleConv Training')
parser.add_argument('--dataset', default='cifar10', type=str,
                    help='dataset (cifar10 [default] or cifar100)')
parser.add_argument('--arch', default='NetSimpleConv', type=str,
...
parser.add_argument('-c2', '--class2', default=-1, type=int,
...
def main():
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        # cifar10 (50K training, 10K validation)
        trainds = datasets.__dict__[args.dataset.upper()](os.path.expanduser('~/.torch/data'), train=True,
                                                          download=True, transform=transform_train)
        valds = datasets.__dict__[args.dataset.upper()](os.path.expanduser('~/.torch/data'), train=False,
                                                        transform=transform_test)
        # numClass =  args.dataset == 'cifar10' and 10 or 100
        numClass = 10
    ...
    if args.arch == 'wideresnet':
        model = WideResNet(args.layers, numClass, args.widen_factor, dropRate=args.droprate, init_scale=args.init_scale,
                           init_type=args.init_type)  # std for normal initialization

    elif args.arch.startswith('NetSimpleConv') and args.arch != 'NetSimpleConv':
        func = getattr(extarget, args.arch)
        model = func(8 * args.widen_factor, numClass, init_scale=args.init_scale, init_type=args.init_type,
                     has_nonlinear=int(args.hasnonlinear), has_bn=int(args.hasbn),
                     xnorm=args.xnorm)  # n-layer all conv
        # print(model)
    elif args.arch == 'NetSimpleConv':
        model = extarget.NetSimpleConv(8 * args.widen_factor, numClass, init_scale=args.init_scale,
                                       init_type=args.init_type, has_nonlinear=int(args.hasnonlinear),
                                       has_bn=int(args.hasbn))  # 5-layer all conv
    ...
    #     model = extarget.NetSimpleConv_old(8*args.widen_factor,numClass, init_scale=args.init_scale,init_type=args.init_type, has_nonlinear= int(args.hasnonlinear) , has_bn=int(args.hasbn) ) # 5-layer all conv
        ...
        model = extarget.NetSimpleConv_more(8 * args.widen_factor, numClass, init_scale=args.init_scale,
                                            init_type=args.init_type, nlayerMore=args.nlayerMore)  # 5-layer all conv
    elif args.arch == 'NetSimpleConvNOBN':
        model = extarget.NetSimpleConvNOBN(8 * args.widen_factor, numClass, init_scale=args.init_scale,
                                           init_type=args.init_type)  # 5-layer all conv , no batch norm
    elif args.arch == 'LinearNet':
        model = extarget.LinearNet(16 * args.widen_factor, numClass, init_scale=args.init_scale,
                                   init_type=args.init_type)  # linear net
    elif args.arch == 'LinearNetWithBN':
        model = extarget.LinearNetWithBN(16 * args.widen_factor, numClass, init_scale=args.init_scale,
                                         init_type=args.init_type, has_bn=int(args.hasbn))  # linear net
    elif args.arch == 'HiddenLayer_1':
        model = extarget.HiddenLayer_1(32 * args.widen_factor, numClass, init_scale=args.init_scale,
                                       init_type=args.init_type)  # linear net
    elif args.arch == 'HiddenLayer_2':
        model = extarget.HiddenLayer_2(8 * args.widen_factor, numClass, init_scale=args.init_scale,
                                       init_type=args.init_type, has_bn=int(args.hasbn), affine=args.bn_affine)  # conv
    elif args.arch == 'HiddenLayer_2_FC':
        model = extarget.HiddenLayer_2_FC(32 * args.widen_factor, numClass, init_scale=args.init_scale,
                                          init_type=args.init_type, has_bn=int(args.hasbn), affine=args.bn_affine,
                                          xnorm=args.xnorm)  # fc
    elif args.arch == 'HiddenLayer_3_FC':
        model = extarget.HiddenLayer_3_FC(32 * args.widen_factor, numClass, init_scale=args.init_scale,
                                          init_type=args.init_type, has_bn=int(args.hasbn), affine=args.bn_affine,
                                          xnorm=args.xnorm)  # fc
    elif args.arch == 'HiddenLayer_1_NOBN':
        model = extarget.HiddenLayer_1_NOBN(32 * args.widen_factor, numClass, init_scale=args.init_scale,
                                            init_type=args.init_type)  # linear net
    elif args.arch == 'NetSimpleConvNorm1x':
        model = extarget.NetSimpleConvNorm1x(8 * args.widen_factor, numClass, init_scale=args.init_scale,
                                             init_type=args.init_type)  # 5-layer all conv
    # elif args.arch == 'NetSimpleConvNOBN':
    ...
    import extarget
    extarget.init_convnet(model, init_scale=args.init_scale, init_type=args.init_type, bn_affine=args.bn_affine,
                          bnpost=args.bnpost)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #    'train_margin': tr_margin,
        # }, is_best, 'checkpoint_epoch%d.pth.tar'%(epoch + 1))
        save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
            'best_prec1': best_prec1,
...
def train(train_loader, model, criterion, optimizer, scheduler, epoch):
    ...
    # print('train_loader: ', enumerate_lookahead(train_loader)[0])
    for i, (input, target), has_more in enumerate_lookahead(train_loader):
        # batch_img_index = index.detach().cpu().numpy()
        # batch_img_id_record.append(batch_img_index)
        # exist= K in batch_img_index
        # if exist:
        # d_ind = torch.where(index == K)

        target = target.cuda(non_blocking=True)
        input = input.cuda(non_blocking=True)
        input = input_proc(model, input)
        output = model(input)
        # print(target, len(train_loader))

        # target = target - 1  # add 5/15 change [1,2]-->[0,1]
        ...
        if args.loss_type == 'MSE':
            ...
            # print(target.size())
            # print(target.reshape(target.size()[0], 1))
            # print(target)
            ...
            # target_final--[batch_size, num_classes]==[128,10]
            target_final = torch.zeros([target.size()[0], num_classes], device=device).scatter_(1, target.reshape(
                target.size()[0], 1), 1)
            # mse_weights = target_final * 4  + 1
            # print(target_final)
            ...
            loss = torch.mean(
                (misc.normalize_input(output.data) - misc.normalize_input(target_final.type(torch.float))) ** 2)
            loss = loss.detach()
        ...
        if len(valid_output_ids) == 2 and args.loss_type == 'MSE':
            import complexity
            # compute prod_rho (product of Frobenius norms of weight matrices)
            prod_rho, conv_values, bn_std, bn_scaling, conv_keys, bn_keys, bn_keys2 = complexity.get_complexities(model,
                                                                                                                  'fro')
            # print('output: ', output)
            ...
            # if len(d_ind[0])==1:
            #   mm = ynfn[d_ind]
            #  mm = mm.clone().detach().cpu().numpy()[0]
            # p = p+i
        ...
        if args.tensorboard and epoch <= 40:
            ...
            if args.tensorboard and epoch <= 30:
                if args.tensorboard and epoch <= 3:
                    ...
                    # summary_writer.add_scalar('minibatch_mean_abs_fn_by_iter_less',
                    #                           (abs_fn / current_minibatch_size).detach().cpu().numpy(), current_i)
                ...
                if ynfn is not None:
                    summary_writer.add_scalar('minibatch_mean_ynfn_by_iter',
                                              (ynfn / current_minibatch_size).detach().cpu().numpy(), current_i)
                    summary_writer.add_scalar('minibatch_mean_fn2_by_iter',
                                              (fn2 / current_minibatch_size).detach().cpu().numpy(), current_i)
                    summary_writer.add_scalar('minibatch_mean_abs_fn_by_iter',
                                              (abs_fn / current_minibatch_size).detach().cpu().numpy(), current_i)
                    summary_writer.add_scalar('minibatch_mean_ynfn_div_fn2_by_iter',
                                              (ynfn / fn2).detach().cpu().numpy(), current_i)
            ...
            if args.tensorboard and epoch <= 5:
                ...
                # print(type(prec1))
                # import pdb; pdb.set_trace()
                summary_writer.add_histogram('output_by_iter',
                                             output[:, valid_output_ids].clone().detach().cpu().numpy(), current_i)
    ...
    if args.tensorboard:
        ...
        if len(ynfn_buffer) > 0:
            ynfn_buffer = torch.cat(ynfn_buaffer)
            import h5py
    ...
    # save check point for every epoch
    # tr_margin = ynfn_buffer.detach().cpu().numpy()
    # return tr_margin
...
def validate(val_loader, model, criterion, epoch):
    ...
    for i, (input, target), has_more in enumerate_lookahead(val_loader):
        target = target.cuda(non_blocking=True)
        input = input.cuda(non_blocking=True)
        # target = target - 1
